...
# 数据处理和分析函数
def process_data(actual_df, forecast_df, product_info_df, selected_months=None, selected_regions=None):
    """处理数据并计算关键指标，支持月份和区域筛选"""
    # 按月份、区域、产品码汇总数据
    ...
    # 计算总体准确率 - 使用支持筛选的函数
    national_accuracy = calculate_national_accuracy(merged_monthly, selected_months, selected_regions)
    regional_accuracy = calculate_regional_accuracy(merged_monthly, selected_months, selected_regions)
...
def calculate_national_accuracy(merged_df, selected_months=None, selected_regions=None):
    """计算全国的预测准确率，支持月份和区域筛选"""
    # 首先应用筛选
    filtered_df = merged_df.copy()
    if selected_months and len(selected_months) > 0:
        filtered_df = filtered_df[filtered_df['所属年月'].isin(selected_months)]
    if selected_regions and len(selected_regions) > 0:
        filtered_df = filtered_df[filtered_df['所属区域'].isin(selected_regions)]

    # 按月份汇总
    monthly_summary = filtered_df.groupby('所属年月').agg({
        '求和项:数量（箱）': 'sum',
        '预计销售量': 'sum'
    }).reset_index()
...
def calculate_regional_accuracy(merged_df, selected_months=None, selected_regions=None):
    """计算各区域的预测准确率，支持月份和区域筛选"""
    # 首先应用筛选
    filtered_df = merged_df.copy()
    if selected_months and len(selected_months) > 0:
        filtered_df = filtered_df[filtered_df['所属年月'].isin(selected_months)]
    ...
    # 按月份和区域汇总
    region_monthly_summary = filtered_df.groupby(['所属年月', '所属区域']).agg({
        '求和项:数量（箱）': 'sum',
        '预计销售量': 'sum'
    }).reset_index()
...
def calculate_product_growth(actual_monthly, regions=None, months=None, growth_min=-100, growth_max=500):
    ...
    # 检查是否有足够的数据进行同比计算
    # 修改: 降低对历史数据的要求，只需要有相邻月份数据即可进行计算
...
# 处理数据
processed_data = process_data(actual_data, forecast_data, product_info, selected_months, selected_regions)
...
with tabs[1]:  # 预测差异分析标签页
    ...
    if not diff_selected_months or not diff_selected_regions:
        ...
        # 准备数据 - 使用全国数据
        selected_region_for_diff = '全国'  # 默认使用全国，不再允许选择
