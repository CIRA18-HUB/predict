...
def format_number(value):
    ...
    if value >= 1000000:  # 百万级别
        return f"{value / 1000000:.2f}百万箱"
    elif value >= 10000:  # 万级别
        return f"{value / 10000:.2f}万箱"
    else:
        return f"{value:.0f}箱"
...
with tabs[1]:  # 产品趋势标签页
    ...
    if 'latest_growth' in product_growth and not product_growth['latest_growth'].empty:
        ...
        # 产品增长/下降分析 - 使用水平条形图
        st.markdown('<div class="sub-header">产品三个月滚动同比增长率（含备货建议）</div>', unsafe_allow_html=True)
...
with tabs[2]:  # 重点SKU分析标签页
    ...
    # 全国重点SKU分析
    st.markdown('<div class="sub-header">全国销售量占比80%SKU列表</div>', unsafe_allow_html=True)
    ...
    # 各区域重点SKU分析
    st.markdown('<div class="sub-header">各区域销售量占比80%SKU分析</div>', unsafe_allow_html=True)
...
with tabs[4]:  # 历史趋势标签页
    ...
    for code in all_products:
        product_options.append(format_product_code(code, product_info, include_name=True))
    ...
    # 从显示名称中提取产品代码
    selected_product = selected_product_display.split(' ')[0] if ' ' in selected_product_display else selected_product_display
    ...
    if not product_trend.empty:
        ...
        <b>图表解读：</b> 此图展示了{selected_product_display}产品在{selected_region_for_trend}的销售量趋势和差异率。浅绿色背景区域是当前选定的分析月份。
