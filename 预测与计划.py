...
def display_recommendations_table(latest_growth, product_info):
    ...
    <table class="recommendation-table">
        <thead>
            <tr>
                ...
                <th style="text-align: center;">3个月销量</th>
    ...
    # 计算开始和结束索引
    start_index = st.session_state.recommendation_page * items_per_page
    end_index = min(start_index + items_per_page, len(display_data))
    ...
    for _, row in page_data.iterrows():
        ...
        # 准备建议样式
        recommendation_style = f"class='recommendation-tag {row.get('建议样式类', '')}'"
        ...
        <tr>
            ...
            <td style="text-align: center;">{int(row.get('3个月滚动销量', 0))}</td>
            <td style="text-align: center;" class="{trend_class}">{row['趋势']}</td>
            <td style="text-align: center;">
                <span {recommendation_style}>
                    {row['备货建议']} {row.get('建议图标', '')}
                </span>
            </td>
            <td style="text-align: center;" class="{growth_class}">{row['调整比例']}%</td>
        </tr>
        """
    ...
    # 添加分页控件（如果产品数量超过每页显示数）
    if len(display_data) > items_per_page:
...
# 创建标签页 - 已合并总览和预测差异
tabs = st.tabs(["📊 总览与差异", "📈 产品趋势", "🔍 重点SKU分析", "📉 历史趋势"])

with tabs[0]:  # 总览与差异标签页
    # 在标签页内添加筛选器
    st.markdown("### 📊 分析筛选")
    ...
    if not selected_months or not selected_regions:
        ...
        <b>图表解读：</b> 此图展示了销售量预测的月度准确率趋势。蓝线表示销售量准确率，绿色虚线代表理想准确率目标线(85%)。
        ...
        if selected_months:
            ...
            latest_region_accuracy = region_accuracy_monthly[
                ...
                (region_accuracy_monthly['所属区域'].isin(selected_regions))
                ].copy()
            ...
            if not latest_region_accuracy.empty:
                ...
                # 生成动态解读
                explanation_text = "<b>图表解读：</b> 此图比较了各区域销售量预测的准确率，绿色虚线代表理想准确率目标(85%)。"
                ...
                st.warning(f"没有找到{latest_selected_month}的区域准确率数据。")

        # 预测与实际销售对比
        st.markdown("### 预测与实际销售对比")
        ...
        if selected_region_for_diff == '全国':
            ...
            region_filtered_salesperson = filtered_salesperson[
                filtered_salesperson['所属区域'] == selected_region_for_diff]
        ...
        if not diff_data.empty:
            ...
            # 生成动态解读
            diff_explanation = f"""
            <b>图表解读：</b> 此图展示了各{analysis_dimension}的实际销售量(蓝色)与预测销售量(红色)对比，点的颜色表示差异率(绿色表示低估，红色表示高估)。
            差异率越高(绝对值越大)，表明预测偏离实际的程度越大。
            """
            ...
            if not top_diff_items.empty:
                ...
                if analysis_dimension == '产品':
                    ...
                    if not high_diff_items.empty:
                        ...
                        diff_explanation += f"各{analysis_dimension}预测与实际销售较为匹配，建议保持当前预测方法。"
...
with tabs[1]:  # 产品趋势标签页
    ...
    if not trend_selected_months or not trend_selected_regions:
        ...
        if 'latest_growth' in product_growth and not product_growth['latest_growth'].empty:
            ...
            # 各区域产品增长情况
            st.markdown("### 各区域产品增长分析")

            # 选择区域
            if trend_selected_regions:
                selected_region_for_growth = st.selectbox(
                    "选择区域查看产品增长情况",
                    options=trend_selected_regions,
                    index=0
                )

                # 提取该区域当月数据
                if trend_selected_months:
                    region_month_data = trend_filtered_monthly[
                        trend_filtered_monthly['所属区域'] == selected_region_for_growth].copy()
                    ...
                    if not region_month_data.empty:
                        ...
                        region_products_with_growth = pd.merge(
                            region_products,
                            latest_growth[
                                ['产品代码', '销量增长率', '趋势', '备货建议', '调整比例', '建议颜色', '建议图标']],
                            on='产品代码',
                            how='left'
                        )

                        # 使用优化的散点图函数
                        fig_region_growth = create_improved_scatter(
                            data=region_products_with_growth,
                            x='求和项:数量（箱）',
                            y='销量增长率',
                            size='求和项:数量（箱）',
                            color='趋势',
                            title=f"{selected_region_for_growth}区域产品销量与增长率分析",
                            hover_name='产品显示'
                        )
                        ...
                        if not region_products_with_growth.empty:
                            ...
                            if not top_growth_products.empty:
                                ...
                                region_explanation += f"{product['产品显示']}是增长型高销售量产品(增长率{product['销量增长率']:.1f}%)，建议{product['备货建议']}；"

                            if not top_decline_products.empty:
                                product = top_decline_products.iloc[0]
                                region_explanation += f"{product['产品显示']}是下降型高销售量产品(增长率{product['销量增长率']:.1f}%)，建议{product['备货建议']}。"
                        ...
                        st.warning(f"没有找到{selected_region_for_growth}区域的产品数据。")
                else:
                    st.warning("请选择至少一个月份进行分析。")
            else:
                st.warning("请选择至少一个区域进行分析。")
        else:
...
with tabs[2]:  # 重点SKU分析标签页
    ...
    # 使用全部月份数据，仅按区域筛选
    sku_filtered_monthly = filter_data(processed_data['merged_monthly'], all_months, sku_selected_regions)

    # 检查筛选条件是否有效
    if not sku_selected_regions:
        st.warning("请选择至少一个区域进行分析。")
    else:
        ...
        # 各区域重点SKU分析
        st.markdown("### 各区域销售量占比80%SKU分析")

        # 选择区域
        if len(sku_selected_regions) > 0:
            selected_region_for_sku = st.selectbox(
                "选择区域查看重点SKU",
                options=sku_selected_regions,
                index=0,
            ...
            if selected_region_for_sku in regional_top_skus and not regional_top_skus[selected_region_for_sku].empty:
                ...
                # 创建区域和全国重点SKU的名称映射
                common_sku_names = [format_product_code(code, product_info) for code in common_skus]
                region_unique_sku_names = [format_product_code(code, product_info) for code in region_unique_skus]
                national_unique_sku_names = [format_product_code(code, product_info) for code in national_unique_skus]

                # 准备悬停文本
                hover_texts = [
                    f"共有SKU: {len(common_skus)}个<br>" +
                    f"包含: {', '.join(common_sku_names[:3]) + ('...' if len(common_sku_names) > 3 else '')}",

                    f"区域特有SKU: {len(region_unique_skus)}个<br>" +
                    f"包含: {', '.join(region_unique_sku_names[:3]) + ('...' if len(region_unique_sku_names) > 3 else '')}",

                    f"全国重点非区域SKU: {len(national_unique_skus)}个<br>" +
                    f"包含: {', '.join(national_unique_sku_names[:3]) + ('...' if len(national_unique_sku_names) > 3 else '')}"
                ]
                ...
                <b>图表解读：</b> 此饼图展示了{selected_region_for_sku}区域重点SKU与全国重点SKU的对比情况。共有SKU(绿色)表示同时是区域和全国重点的产品；区域特有SKU(蓝色)表示只在该区域是重点的产品；全国重点但区域非重点SKU(红色)表示在全国范围内是重点但在该区域不是重点的产品。
                <br><b>SKU详情：</b> 共有SKU: {', '.join([format_product_code(code, product_info) for code in list(common_skus)[:3]])}等{len(common_skus)}个；
                区域特有SKU: {', '.join([format_product_code(code, product_info) for code in list(region_unique_skus)[:3]])}等{len(region_unique_skus)}个；
                全国重点非区域SKU: {', '.join([format_product_code(code, product_info) for code in list(national_unique_skus)[:3]])}等{len(national_unique_skus)}个。
            ...
            st.warning("请选择至少一个区域以查看区域SKU分析。")
    ...
    with tabs[3]:  # 历史趋势标签页
        ...
        with st.expander("筛选条件", expanded=True):
            col1, col2 = st.columns(2)
            with col1:
                trend_tab_selected_months = st.multiselect(
                    "选择分析月份",
                    options=all_months,
                    default=all_months,  # 默认选择全部月份用于趋势分析
                    key="trend_tab_months"
                )

            with col2:
                trend_tab_selected_regions = st.multiselect(
                    "选择区域",
                    options=all_regions,
                    default=all_regions,
                    key="trend_tab_regions"
                )

        # 筛选数据
        trend_tab_filtered_monthly = filter_data(processed_data['merged_monthly'], trend_tab_selected_months,
                                                 trend_tab_selected_regions)

        # 检查筛选条件是否有效
        if not trend_tab_selected_months or not trend_tab_selected_regions:
            st.warning("请选择至少一个月份和一个区域进行分析。")
        else:
            st.markdown("### 销售与预测历史趋势")

            # 准备历史趋势数据
            monthly_trend = trend_tab_filtered_monthly.groupby(['所属年月', '所属区域']).agg({
                '求和项:数量（箱）': 'sum',
                '预计销售量': 'sum'
            }).reset_index()
            ...
            selected_region_for_trend = st.selectbox(
                "选择区域查看历史趋势",
                options=['全国'] + trend_tab_selected_regions,
                index=0,
            ...
            # 强调选定月份
            if trend_tab_selected_months:
                for month in trend_tab_selected_months:
    ...
    st.markdown("""
                <div style="margin-top: 50px; padding-top: 20px; border-top: 1px solid #eee; text-align: center; color: #666; font-size: 0.8rem;">
                    <p>销售预测准确率分析仪表盘 | 版本 2.1.0 | 最后更新: 2025年4月</p>
