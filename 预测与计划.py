...
def create_improved_scatter(data, x, y, size, color, title, hover_name=None):
    ...
    # 改进轴线和网格
    fig.update_xaxes(showgrid=True, gridwidth=0.5, gridcolor='rgba(220,220,220,0.5)')
    fig.update_yaxes(showgrid=True, gridwidth=0.5, gridcolor='rgba(220,220,220,0.5)')
...
with tabs[1]:  # 产品趋势标签页
    ...
    if not trend_selected_months or not trend_selected_regions:
        ...
        if 'latest_growth' in product_growth and not product_growth['latest_growth'].empty:
            ...
            # 产品增长/下降分析 - 使用水平条形图
            st.markdown("### 产品三个月滚动同比增长率（含备货建议）")
            ...
            # 添加图表解释
            growth_explanation = """
            <b>图表解读：</b> 此图展示了各产品三个月滚动销量的同比增长率，按增长率从高到低排序。颜色代表增长趋势：深蓝色为强劲增长(>10%)，绿色为增长(0-10%)，橙色为轻微下降(0--10%)，红色为显著下降(<-10%)。
            ...
            if trend_selected_regions:
                ...
                if trend_selected_months:
                    ...
                    if not region_month_data.empty:
                        ...
                        # 只为前3个重要产品添加标签，避免拥挤
                        for i, row in region_products_with_growth.nlargest(3, '求和项:数量（箱）').iterrows():
        ...
        if len(sku_selected_regions) > 0:
            ...
            if selected_region_for_sku in regional_top_skus and not regional_top_skus[selected_region_for_sku].empty:
                ...
                sku_comparison_explanation = f"""
                                ...
                                <br><b>SKU详情：</b> 共有SKU:{', '.join([format_product_code(code, product_info) for code in common_skus][:3])}等{len(common_skus)}个；
                                区域特有SKU:{', '.join([format_product_code(code, product_info) for code in region_unique_skus][:3])}等{len(region_unique_skus)}个；
                                全国重点非区域SKU:{', '.join([format_product_code(code, product_info) for code in national_unique_skus][:3])}等{len(national_unique_skus)}个。
