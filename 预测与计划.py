...
def generate_dynamic_title(base_title, include_filters=False):
    ...
    # 根据需要添加筛选信息，但避免在图表内部重复
    if include_filters and 'global_filters' in st.session_state:
        filters = st.session_state.global_filters

        # 只有在不是默认值（所有选择）时才添加筛选信息
        if filters.get('regions') and len(filters['regions']) < len(all_regions):
            regions_str = '、'.join(filters['regions'])
            title += f" ({regions_str})"
...
def display_recommendations_table(latest_growth, product_info):
    ...
    for _, row in page_data.iterrows():
        ...
        # 备货建议样式类
        recommendation_class = row.get('建议样式类', '')
        recommendation_icon = row.get('建议图标', '')
        ...
        <tr>
            ...
            <td style="text-align: center;">
                <span class="recommendation-tag" style="{recommendation_class}">
                    {row['备货建议']} {recommendation_icon}
                </span>
            </td>
...
if 'global_filters' not in st.session_state:
    ...
    # 更新全局筛选状态
    st.session_state.global_filters['months'] = selected_months
    st.session_state.global_filters['regions'] = selected_regions
...
with tabs[0]:  # 总览标签页
    ...
    if selected_months:
        ...
        st.warning("请在左侧筛选器中选择分析月份以查看区域准确率比较。")
    ...
    if 'latest_growth' in product_growth and not product_growth['latest_growth'].empty:
        ...
        if selected_regions:
            ...
            if selected_months:
                ...
                st.warning("请在左侧筛选器中选择分析月份以查看产品增长分析。")
        else:
            st.warning("请在左侧筛选器中选择区域以查看产品增长分析。")
    else:
...
with tabs[2]:  # 重点SKU分析标签页
    ...
    if len(selected_regions) > 0:
        ...
        if selected_region_for_sku in regional_top_skus and not regional_top_skus[selected_region_for_sku].empty:
            ...
            # 更新布局
            title = f"销售量占比80%的SKU及其准确率"  # 简化标题
        ...
        st.warning("请在左侧筛选器中选择至少一个区域以查看区域SKU分析。")
    ...
    with tabs[3]:  # 预测差异分析标签页
        ...
        # 差异分析图表
        display_section_title(f"预测差异分析")
        ...
        if not diff_data.empty:
            ...
            # 更新布局
            title = f"{analysis_dimension}预测与实际销售对比 (销售量前15)"
            ...
            if not top_diff_items.empty:
                ...
                # 占比差异分析
                display_section_title(f"销售占比差异分析")
                ...
                fig_share_diff = px.scatter(
                    ...
                    title=f"{analysis_dimension}销售占比差异分析",
                ...
                # 占比差异排名 - 使用水平条形图
                display_section_title(f"占比差异排名")
                ...
                fig_share_diff_ranking.update_layout(
                    title=f"{analysis_dimension}销售占比差异排名",
                ...
                if selected_region_for_diff != '全国' and analysis_dimension == '销售员':
                    display_section_title(f"销售员产品差异热图")

                    # 获取该区域数据
                    region_data = filtered_salesperson[
                        filtered_salesperson['所属区域'] == selected_region_for_diff].copy()
                    ...
                    if not salesperson_data.empty:
                        ...
                        fig_heatmap.update_layout(
                            title=f"{selected_salesperson} 产品占比对比",
                            xaxis_title="占比 (%)",
                            yaxis_title="产品",
                            barmode='group',
                            xaxis=dict(
