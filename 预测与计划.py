...
def simplify_product_name(code, full_name):
    ...
    # 去除"口力"前缀和"-中国"后缀
    name = full_name.replace("口力", "").replace("-中国", "")

    # 提取主要产品名称和规格信息
    try:
        import re
        # 匹配产品主要名称和规格
        match = re.search(r'([^0-9]+)([0-9]+[Gg])?', name)
        if match:
            product_type = match.group(1).strip()
            spec = match.group(2) or ""
            return f"{code}{product_type}{spec}"
    except Exception:
        pass

    # 如果正则匹配失败，返回代码+名称的简单组合
    return f"{code}{name}"
...
def format_product_code(code, product_info_df, include_name=True):
    ...
    if include_name:
        # 优先使用简化名称
        if '简化产品名称' in product_info_df.columns:
            filtered_df = product_info_df[product_info_df['产品代码'] == code]
            if not filtered_df.empty and not pd.isna(filtered_df['简化产品名称'].iloc[0]):
                return filtered_df['简化产品名称'].iloc[0]
...
def calculate_national_accuracy(merged_df):
    ...
    monthly_summary['销售额准确率'] = monthly_summary.apply(
        lambda row: calculate_unified_accuracy(row['销售额'], row['预测销售额']),
        axis=1
    )
    ...
    overall = {
        ...
        '销售额准确率': safe_mean(monthly_summary['销售额准确率'], 0)
    }
...
def calculate_regional_accuracy(merged_df):
    ...
    region_monthly_summary['销售额准确率'] = region_monthly_summary.apply(
        lambda row: calculate_unified_accuracy(row['销售额'], row['预测销售额']),
        axis=1
    )
                        ...
                        if current_3m_sum > 0 and prev_3m_sum > 0 and len(months_current) > 0 and len(months_prev) > 0:
                            if prev_3m_sum > 0:
                                try:
                                    ...
                                    # 限制异常值，防止极端增长率
                                    growth_rate = max(min(growth_rate, 1000), -100)  # 限制增长率在-100%到1000%之间
                            ...
                            if current_3m_amount > 0 and prev_3m_amount > 0:
                                if prev_3m_amount > 0:
                                    try:
                                        ...
                                        # 限制异常值
                                        amount_growth_rate = max(min(amount_growth_rate, 1000), -100)
                                    except (OverflowError, ZeroDivisionError, ValueError):
                                        amount_growth_rate = 0
                                else:
                                    amount_growth_rate = 0 if current_3m_amount == 0 else 100
    ...
    if not growth_df.empty:
        try:
            ...
            if not latest_growth.empty:
                ...
                # 同样处理销售额趋势判断
                latest_growth = latest_growth[latest_growth['销售额增长率'].notna()]
                latest_growth = latest_growth[np.isfinite(latest_growth['销售额增长率'])]
...
def calculate_top_skus(merged_df, by_region=False):
    ...
    if by_region:
        ...
        for region in grouped['所属区域'].unique():
            ...
            region_data['累计占比'] = region_data['累计销售额'] / total_sales * 100 if total_sales > 0 else 0
        ...
        grouped['累计占比'] = grouped['累计销售额'] / total_sales * 100 if total_sales > 0 else 0
            ...
            if not product_trend.empty:
                ...
                if len(product_trend) > 1:
                    ...
                    if qty_trend > 0 and accuracy_mean < 80:
                        ...
                        if qty_trend < 0 and consecutive_over:
                            try:
                                # 使用安全计算方法避免溢出错误
                                if not recent_months.empty and '数量差异率' in recent_months.columns:
                                    diff_values = recent_months['数量差异率'].dropna()
                                    if not diff_values.empty:
                                        adjust_pct = abs(round(safe_mean(diff_values, 0)))
                                        product_explanation += f"对于销量下降且持续高估的{selected_product_display}，建议降低预测量{adjust_pct}%以避免库存积压；"
                                    else:
                                        product_explanation += f"对于销量下降且持续高估的{selected_product_display}，建议降低预测量以避免库存积压；"
                                else:
                                    product_explanation += f"对于销量下降且持续高估的{selected_product_display}，建议降低预测量以避免库存积压；"
                            except Exception:
                                # 发生任何错误时使用默认文本
